...
bool queue_full(queue_t* q){
    ...
    if(incrementIndex(q, q->indexIn) == q->indexOut){
...
queue_data_t queue_readElementAt(queue_t* q, queue_index_t index){
    ...
    //Checks to see if the index will go over
    if((index + q->indexOut) > (q->size - INDEX_FIX)){

        //Returns the element
        return q->data[q->size - q->indexOut - index];
    }
