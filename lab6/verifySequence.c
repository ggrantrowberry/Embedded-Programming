...
#define INIT_VALUE 0 //Initial value for many things
#define TIME_OUT_MAX 100 //Time out max value
#define SEQ_LENGTH_FIX 1 //To make it so the length is -1
#define ERASE true


//Determines whether or not the state machine is on
static bool verifySequenceEnabled = true;

//If there is a time out error
static bool isTimeOutErrorFlag = false;

//If there is a input error
static bool isUserInputErrorFlag = false;

//If the sequence is complete
static bool isCompleteFlag = false;
...
enum verifySequence_st_t {
    ...
    get_input_st, //Flashes the pattern
    check_input_st, //Checks the input
    end_st //The final state
} verifySequenceCurrentState;
...
void debugStatePrintVS() {
  ...
  if (previousState != verifySequenceCurrentState || firstPass) {
    firstPass = false;                // previousState will be defined, firstPass is false.
    previousState = verifySequenceCurrentState;     // keep track of the last state that you were in.
    //printf("msCounter:%d\n\r", msCounter);
    switch(verifySequenceCurrentState) {            // This prints messages based upon the state that you were in.
      case init_st:
        printf("VS_init_st\n\r");
        break;
      case get_input_st:
        printf("VS_get_input_st\n\r");
        break;
      case check_input_st:
        printf("VS_check_input_st\n\r");
        break;
      case end_st:
        printf("VS_end_st\n\r");
        break;
...
//TimeOutCount
static uint8_t timeOutCount = INIT_VALUE;

//Sequence iterator
static uint16_t sequenceCount = INIT_VALUE;

//Last region touched
uint8_t regionNumber = INIT_VALUE;

// Initialize the state machine
void verifySequence_init(){

    //Sets the current state to init
    verifySequenceCurrentState = init_st;
         ...
         //Initial state
           case init_st:
               ...
               if(verifySequenceEnabled){
                   ...
                   //Sets the current state to wait for input
                   verifySequenceCurrentState = get_input_st;

                   //Sets the time out to zero
                   timeOutCount = INIT_VALUE;

                   //Sets the sequenceCount
                   sequenceCount = INIT_VALUE;

                   //Sets the complete flag
                   isCompleteFlag = false;

                   //If there is a time out error
                   isTimeOutErrorFlag = false;

                   //If there is a input error
                   isUserInputErrorFlag = false;
           ...
           case get_input_st:
               ...
               if(!verifySequenceEnabled){
                   ...
                   //Checks to see if it has timed out
                   if(timeOutCount == TIME_OUT_MAX){

                       //Sends to error
                       verifySequenceCurrentState = end_st;

                       //Sets the timeOutErrorFlag
                       isTimeOutErrorFlag = true;

                       //Disables the buttonHandler
                   ...
                   //Checks to see if there is a release
                   if(buttonHandler_releaseDetected()){
                       ...
                       //Changes the state
                       verifySequenceCurrentState = check_input_st;
           ...
           case check_input_st:
               ...
               if(!verifySequenceEnabled){
                   ...
                   //Checks to see if the region touched is correct
                   if(globals_getSequenceValue(sequenceCount) == regionNumber){

                       //Checks to see if the sequence is done
                       if(sequenceCount == globals_getSequenceIterationLength()- SEQ_LENGTH_FIX){

                           //If it is done then send to end_st
                           verifySequenceCurrentState = end_st;

                           //Sets the isCompleteFlag to true
                           isCompleteFlag = true;

                       } else {

                           //Send back to get input
                           verifySequenceCurrentState = get_input_st;
                       ...
                       //Sends it to the end with error
                       verifySequenceCurrentState = end_st;

                       //Sets the input error flag
                       isUserInputErrorFlag = true;
